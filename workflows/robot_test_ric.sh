...
FILE= test
if find "$FILE" -mindepth 1 -print -quit 2>/dev/null | grep -q .; then
    cp test/ric_robot_suite/helm/robot_install.sh /home/cloudadmin/RIC/dep/generated/ricplt
else
    echo "'$FILE' is empty or not a directory"
    exit 1
fi
...
DEPLOY=ric-robot
while [ ${TIMEOUT} -gt 0 -a ${STATUS} != 'True' ]; do  sleep 1  TIMEOUT=`expr ${TIMEOUT} - 1`
 STATUS=`kubectl -n ${NAMESPACE} get deployment ${DEPLOY} \
         -o template \
         --template='{{if (or .status.readyReplicas 0) | eq .status.replicas}}True{{else}}False{{end}}'`
done
if [[ $STATUS != 'True' ]]; then
 exit 1
fi
